...
- `<FannedCards>` — cards fan out into a tilted spread on scroll (the dark "story lessons" section; also the keepsake hero book mocks). Spacing tightens as the card count grows; **hovering a card lifts it out of the hand** — cancels its tilt, scales `1.06`, rises, and jumps to the front (`z-index 999`). Optional `minHeight` (default `620`) and `spread` (multiplier on the fan's x/y offsets, default `1`) fit tighter stacks — the keepsake hero uses `minHeight={380} spread={0.55}`.
