...
- `<FeatureZoom cards=… head=…>` — the pinned feature row **and** its hand-off into the dark stories section, in one scroll timeline: intro cards pan left, then the final card slides to centre and zooms to full-bleed while a `--twilight-deep` veil closes over it → seamless black into `.u-stories`. Degrades to a plain horizontal-scroll row under reduced motion. Classes: `.u-fzoom` (360vh pin), `.u-fzoom-sticky`, `.u-fzoom-track` (panning intro cards), `.u-fzoom-finale` / `.u-fzoom-media` (the zooming last card), `.u-fzoom-veil`.
  - **Feature section on small screens:** at ≤720 the horizontal pan can't fit (cards overflow, captions clip), so `FeatureZoom` renders `<FeatureZoomMobile>` (`src/components/motion/FeatureZoomMobile.tsx`) instead — a pinned **3D card deck**. The section pins (`.u-fzoom-m` = N×100vh, `.u-fzoom-m-sticky`); a `perspective`/`preserve-3d` stage (`.u-fzoom-m-stage`) holds N media tiles (`.u-fzoom-m-tile`, one grid cell). A card's signed distance from the front is `d = deckPos(p,n) - i`, where `deckPos` **dwells** at each integer for ~55% of its segment (card held readable) then transitions (the deal). `d<0`: receded into depth (small, tilted, −Z) rising to front; `d=0`: front, upright; `d>0` (non-last): dealt — flips up + tumbles toward the camera (rotateX + +Z) fading out as the next pops forward (preserve-3d sorts by translateZ so the dealt tile passes in front). Captions live in a **fixed slot below** the deck (`.u-fzoom-m-capslot` / `.u-fzoom-m-cap`), swapping one at a time (opacity by `|d|`) so they never collide or ride the tiles — a brief captionless beat covers each flip. The last tile zooms its media to black (reusing the desktop hand-off) into `.u-stories`; head fades before it; an ambient orange glow (`.u-fzoom-m-stage::before`) warms the stage. Reduced motion still gets `FeatureZoom`'s static scroll-row. **All scroll-linked opacity uses the function form of `useTransform`** (never the `[in]→[out]` array form) — the array form compiles to a WAAPI ViewTimeline keyed to the element's own non-monotonic viewport progress and flickers; see the note in `FeatureZoom.tsx`.
