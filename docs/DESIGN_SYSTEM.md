...
- `<FeatureZoom cards=… head=…>` — the pinned feature row **and** its hand-off into the dark stories section, in one scroll timeline: intro cards pan left, then the final card slides to centre and zooms to full-bleed while a `--twilight-deep` veil closes over it → seamless black into `.u-stories`. Classes: `.u-fzoom` (300vh pin), `.u-fzoom-sticky`, `.u-fzoom-track` (panning intro cards), `.u-fzoom-last` / `.u-fzoom-media` (the zooming last card), `.u-fzoom-cap` (its caption), `.u-fzoom-veil`. **Small screens (`≤720px`) drop the pin/pan/zoom entirely** — the pan is too short and the scrubbed caption fade was fragile (framer compiled the array-form `useTransform` opacity onto a native `ViewTimeline`, whose non-monotonic element-progress flickered the caption 1→0→1; scrubbed caption values must therefore use the **function form** of `useTransform`, which stays on the rAF path). Instead FeatureZoom renders `<FeatureCarousel>` (`.u-fzoom-static` + `.u-fcar`): a swipeable **centre-snapping carousel** — the centred card pops to full size (`.u-fcar-item.is-active`, scale 1) while neighbours shrink + dim (scale 0.88, opacity 0.5), with a `.u-fcar-dots` dot pager that mirrors + jumps to the active card. Snapping + focus scaling are **pure CSS** (`scroll-snap-type: x mandatory` + `.is-active` transition); an IntersectionObserver on a thin centre band only tracks the active index — no scroll-scrubbed motion. Hands into `.u-stories` at a clean section edge. **Reduced motion** flattens the focus scaling (all cards full-size) and keeps the wide horizontal-scroll row on non-narrow.
