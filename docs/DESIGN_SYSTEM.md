...
Plus umano elevation tokens on `:root`: `--u-orange: #FF692E`, `--u-card-shadow-sm: 0 8px 24px rgba(0,0,0,.08)`, `--u-card-shadow: 0 12px 32px rgba(0,0,0,.10)`, `--u-card-shadow-lg: 0 16px 48px rgba(0,0,0,.14)`, `--u-radius: 16px`. These three (`-sm → -lg`) are the **only** elevation vocabulary — no hard offset shadow anywhere except `<BookCover>` (§3).
...
- **Soft shadow scale (the whole vocabulary):** small pills / nav pill / chips use `var(--u-card-shadow-sm)` (`0 8px 24px rgba(0,0,0,.08)`); cards use `var(--u-card-shadow)` (`0 12px 32px rgba(0,0,0,.10)`); elevated/floating surfaces (hero mock, notification, footer, featured plan) and the **card hover step** use `var(--u-card-shadow-lg)` (`0 16px 48px rgba(0,0,0,.14)`). Never hardcode a raw `0 Npx Npx rgba(...)` value — reference a token so elevation stays consistent and re-skinnable.
...
- **Interaction (fixed pattern):** buttons/pills lift `translateY(-1px)` on hover with the **shadow unchanged** (no shadow-grow) and `translateY(0)` on press (no compression); cards lift `translateY(-2px)` and step to `var(--u-card-shadow-lg)`. Scroll drives the primary motion, not hover.
...
- **Spacing scale (8px grid — CSS variables on `:root`):** `--sp-1: 4px` · `--sp-2: 8px` · `--sp-3: 12px` · `--sp-4: 16px` · `--sp-5: 20px` · `--sp-6: 24px` · `--sp-7: 32px` · `--sp-8: 40px` · `--sp-9: 48px` · `--sp-10: 80px`. Use only these values for new gaps and padding. Off-scale values (`14px`, `18px`, `28px`, `30px`, `36px`) are **non-conforming** — new code must not introduce them.
- **Page width:** `max-width: 1400px`, centered. Section padding `100px 48px`; hero `40px 48px 120px`; app pages `20px 48px 80px` (`--sp-5 --sp-9 --sp-10`). App pages set inline `padding` on `<main class="app-main">` so `.app-main` media rules can override the side values responsively. Exception: keepsake page uses `48px` top padding after its scroll-pinned hero.
- **Grid gaps:** large two-column layouts `40px` (`--sp-8`); medium column grids `20–32px`; card picker grids `16–20px`; chip/item lists `8–12px`. Floating nav link row: `32px`. Builder form-step header, progress pill, option grid, action bar, estimator, gen-banner: `16px`. Portrait + field row, keepsake book picker grid: `20px`.
- **Button sizes:** `.btn` (base `11px 22px`, 16px) → `.btn-sm` (`8px 16px`, 13.5px) → `.btn-lg` (`14px 28px`, 17px). Add `.btn-sm` for compact actions inside cards or toolbars — never override `.btn` padding inline.
...
### Buttons — `.btn`
Pill (radius `43px`), **borderless, no shadow** (flat umano fill — black default, orange CTA), weight `600–800`, hover `translateY(-1px)`, press `translateY(0)`.
...
- **Illustration cover (`coverUrl`):** when set, the story's generated cover art (`next/image` `fill`, `objectFit: cover`, host `res.cloudinary.com`) fills the box behind a top+bottom **ink scrim** (`linear-gradient` of `rgba(20,18,23,·)`), and text is forced legible — cream `label`/`title`, moon `script`, translucent-white star bubble + spine — regardless of the fallback `accent`. Absent → the solid accent block (unchanged). The ink border, hard offset shadow, radius, and spine hairline are kept. Wired only on the full shelf (`storyToBook` passes `book.coverUrl`); dashboard/keepsake stay solid until opted in.
...
- `.kid-pick` (existing-hero picker on `stories/new`): grey **flex row** — a round `.kid-pick-av` avatar (40px, ink border, `overflow:hidden`) holding the hero's generated portrait (`next/image` `fill`) or their initial letter as fallback, then name + meta. Selected = white fill + **inset orange ring** + soft shadow — the same selection language as the keepsake story picker.
...
- `.dash-kid-del` (dashboard hero cards) — small (28px) ink-bordered circular delete affordance pinned in the card corner. Hidden (`opacity:0`) until the card is hovered / the button is focused (always visible on touch via `@media (hover:none)`); hover = **berry fill + white** + hover-lift (destructive intent, reusing the selection accent). Confirms via `window.confirm` then `DELETE /api/children/:id`. Reuse this class for any per-item destructive corner action; don't hand-roll another.
...
Named animation loops (keep names; reuse rather than redefine): `twinkle` / `twinkle2` / `starTwinkle` (star pulse), `fireflyDrift` + `lanternGlow` (drifting lantern glow), `pulseDot` (three-dot wait), plus the umano set (`notifyFloat`, `bookRise`, `badgeWiggle`, `craftPulse`, `headFadeUp`, `shelfPlankDrop`, `skeletonSheen`, `kp*`…). All ambient loops are 2.4–8s and **must** be disabled under `prefers-reduced-motion: reduce`. *(The v1 loops `float`/`spFloat`/`spTwinkle`/`moonDrift`/`shoot`/`rise`/`fillIn`/`playPulse` were removed with the old storybook markup.)*
...
- Generated chapter art is served through the flip-book reader (`<FlipReader>`, `.illus` full-bleed page at `4:7` to match the `1024×1792` art). *(The v1 `.sp-illustration` `16/10` preview and `.illus-thumbs` strip were removed.)*
