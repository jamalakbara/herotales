...
- `<FannedCards>` — cards fan out into a tilted spread on scroll (the dark "story lessons" section). Spacing tightens as the card count grows; **hovering a card lifts it out of the hand** — cancels its tilt, scales `1.06`, rises, and jumps to the front (`z-index 999`).
